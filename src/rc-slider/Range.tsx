...
import {IRangePropTypes} from "./PropsType";
...
@observer
class Range extends Component<IRangePropTypes, State> {
    ...
    @observable store = ((props) => {
        ...
        const bounds = value.map((v) => me.trimAlignValue(v));
    ...
    componentWillReceiveProps(nextProps) {
        ...
        this.changeStore({bounds: nextBounds});
    ...
    onChange(state) {
        ...
        if (isNotControlled) {
            this.changeStore(state);
        } else if (state.handle !== undefined) {
            this.changeStore({handle: state.handle});
        }
    ...
    getBoundNeedMoving(value, closestBound) {
        const {bounds, recent} = this.store;
    ...
    trimAlignValue(v, nextProps = {}) {
        ...
        const valInRange = utils.ensureValueInRange(v, {max: mergedProps.max, min: mergedProps.min});
